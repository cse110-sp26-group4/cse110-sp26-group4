...
import { Issue, ActivityLog, Status, Priority, Action } from "../models/issue.js";
 ...
 * @returns {Issue}
 */
export function createIssue({ title, priority = Priority.LOW, tokenLimit, description } = {}) {
  ...
  const result = db.prepare(`
    ...
    VALUES (?, ?, ?, ?)
  `).run(title?.trim() || "PENDING", priority, tokenLimit ?? null, description ?? null);

  const issue = rowToIssue(db.prepare(`SELECT * FROM issues WHERE id = ?`).get(result.lastInsertRowid));
