...
export function updateIssue(
  id,
  oldIssue,
  { title, description, tokenLimit, status, priority } = {},
) {
  const db = getDB();
  const updates = {};
  ...
  if (status !== undefined) {
    ...
    const toUpdate = statusValues.find(
      (v) => v.trim().toLowerCase() === status.trim().toLowerCase(),
    );
    updates.status = toUpdate || status;
  }
  if (priority !== undefined) {
    const priorityValues = Object.values(Priority);
    const toUpdate = priorityValues.find(
      (v) => v.toLowerCase().trim() === priority.trim().toLowerCase(),
    );
  ...
  if (Object.keys(updates).length > 0)
    db.update(issuesTable).set(updates).where(eq(issuesTable.id, id)).run();
...
export function assignIssue(id, userId) {
  ...
  const assignTx = db.transaction(() => {
    ...
    const newAssignees = Array.isArray(current)
      ? [...current, userId]
      : [userId];
...
export function submitForReview(id) {
  ...
  logActivity(
    ...
    id,
    Action.STATE_CHANGE,
    `Issue #${id} was submitted for review.`,
  );
  return getIssue(id);
}
...
export function getActivityLog(issueId) {
  ...
  return db
    .select()
    .from(activityTable)
    .where(eq(activityTable.issueId, issueId))
    .all();
}
...
export function getRecentActivity({ limit = 20 } = {}) {
  ...
  return db
    .select()
    .from(activityTable)
    .orderBy(sql`${activityTable.logId} DESC`)
    .limit(limit)
    .all();
}
